        ...
        init: function() {
            ...
            switch(this.settings.cssFramework) {
                ...
                case 'foundation':
                    ...
                    break;
                default: 
                    this.selectElements.yearSelect      = $('<select class="dateselector-year"><option value="">Year</option>'), 
                    this.selectElements.monthSelect     = $('<select class="dateselector-month"><option   value="">Month</option>'), 
                    this.selectElements.daySelect       = $('<select class="dateselector-day"><option  value="">Day</option>'),
            ...
            switch(this.settings.cssFramework) {
                ...
                default: 
                    ...
                    for(var M = this.settings.minMonth; M <= this.settings.maxMonth; M++) {
                        console.log("M:::::" + M);
                        var val = M;
                        // console.log("M.length:::::"+val.length);
                        var mVal = M < 10 ? ("0" + M) : "" + M + "";
                        console.log("mVal:::::" + mVal);
                        var monthOption = $('<option value="' + mVal + '">' + monthNames[M] + '</option>');
                        this.selectElements.monthSelect.append(monthOption);
                    }
                    for(var d = this.settings.minDay; d <= this.settings.maxDay; d++) {
                        var dVal= d < 10 ? ("0"+d):""+d+"";
                         console.log("dVal:::::"+dVal);
                        var dayOption = $('<option value="' + dVal + '">' + d + '</option>');
                        this.selectElements.daySelect.append(dayOption);
                    }
                    for(var h = 0; h < 24 / this.settings.hoursStep; h++) {
                        var hour = h * this.settings.hoursStep;
        ...
        _setDateSelectorValue: function(date) {
            ...
            switch(this.settings.cssFramework) {
                case 'bootstrap': case 'foundation':
                    ...
                    break;
                default:
                    this.selectElements.yearSelect.find('option[value="' + date.getFullYear() + '"]');
                    this.selectElements.monthSelect.find('option[value="' + date.getMonth() + '"]');
                    this.selectElements.daySelect.find('option[value="' + date.getDate() + '"]');
