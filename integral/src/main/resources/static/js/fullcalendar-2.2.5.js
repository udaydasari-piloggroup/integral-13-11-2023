...
// Make the mouse cursor express that an event is not allowed in the current area
    function disableCursor() {
        $('body').addClass('visionCalendar-not-allowed');
    }


// Returns the mouse cursor to its original look
    function enableCursor() {
        $('body').removeClass('visionCalendar-not-allowed');
    }
...
// NOTE: this method is best because it takes weird zooming dimensions into account
    function setPotentialScroller(containerEl, height) {
        containerEl.height(height).addClass('visionCalendar-scroller');
...
// Takes an element that might have been a scroller, and turns it back into a normal element.
    function unsetScroller(containerEl) {
        containerEl.height('').removeClass('visionCalendar-scroller');
    }
...
// can be called with another class, or a plain object hash containing new members.
    ...
    var Popover = Class.extend({
        ...
        render: function () {
            ...
            this.el = $('<div class="visionCalendar-popover"/>')
            ...
            // when a click happens on anything inside with a 'visionCalendar-close' className, hide the popover
            this.el.on('click', '.visionCalendar-close', function () {
                _this.hide();
            });
...
// (such as highlight rows, day rows, helper rows, etc).
    ...
    var Grid = fc.Grid = RowRenderer.extend({
        ...
        bindHandlers: function () {
            ...
            this.el.on('mousedown', function (ev) {
                if (
                        !$(ev.target).is('.visionCalendar-event-container *, .visionCalendar-more') && // not an an event element, or "more.." link
                        !$(ev.target).closest('.visionCalendar-popover').length // not on a popover (like the "more.." events one)
                        ) {
        ...
        renderRangeHelper: function (range, sourceSeg) {
            ...
            fakeEvent.className = (fakeEvent.className || []).concat('visionCalendar-helper');
        ...
        highlightSegClasses: function () {
            return ['visionCalendar-highlight'];
        },
        ...
        headHtml: function () {
            return '' +
                    '<div class="visionCalendar-row ' + this.view.widgetHeaderClass + '">' +
                    '<table>' +
                    '<thead>' +
        ...
        headCellHtml: function (cell) {
            ...
            return '' +
                    '<th class="visionCalendar-day-header ' + view.widgetHeaderClass + ' visionCalendar-' + dayIDs[date.day()] + '">' +
        ...
        bgCellHtml: function (cell) {
            ...
            classes.unshift('visionCalendar-day', view.widgetContentClass);
        ...
        getDayClasses: function (date) {
            ...
            var classes = ['visionCalendar-' + dayIDs[date.day()]];

            if (
                    view.name === 'month' &&
                    date.month() != view.intervalStart.month()
                    ) {
                classes.push('visionCalendar-other-month');
            }

            if (date.isSame(today, 'day')) {
                classes.push(
                        'visionCalendar-today',
                        view.highlightStateClass
                        );
            } else if (date < today) {
                classes.push('visionCalendar-past');
            } else {
                classes.push('visionCalendar-future');
            }
    ...
    Grid.mixin({
        ...
        renderFgSegEls: function (segs, disableResizing) {
            ...
            if (segs.length) { // don't build an empty html string
                ...
                $(html).each(function (i, node) {
                    ...
                    if (el) {
                        el.data('visionCalendar-seg', seg); // used by handlers
                        seg.el = el;
                        renderedSegs.push(seg);
                    }
                });
        ...
        bgEventSegClasses: function (seg) {
            ...
            return ['visionCalendar-bgevent'].concat(
                    event.className,
                    source.className || []
                    );
        },
        ...
        businessHoursSegClasses: function (seg) {
            return ['visionCalendar-nonbusiness', 'visionCalendar-bgevent'];
        },
            ...
            $.each(
                    {
                        ...
                        mousedown: function (seg, ev) {
                            if ($(ev.target).is('.visionCalendar-resizer') && view.isEventResizable(seg.event)) {
                    ...
                    function (name, func) {
                        ...
                        _this.el.on(name, '.visionCalendar-event-container > *', function (ev) {
                            var seg = $(this).data('visionCalendar-seg'); // grab segment data. put there by View::renderEvents
        ...
        segDragMousedown: function (seg, ev) {
            ...
            var mouseFollower = new MouseFollower(seg.el, {
                ...
                zIndex: 2 // one above the .visionCalendar-view
            });
        ...
        getSegClasses: function (seg, isDraggable, isResizable) {
            var event = seg.event;
            var classes = [
                'visionCalendar-event',
                seg.isStart ? 'visionCalendar-start' : 'visionCalendar-not-start',
                seg.isEnd ? 'visionCalendar-end' : 'visionCalendar-not-end'
            ].concat(
                    event.className,
            ...
            if (isDraggable) {
                classes.push('visionCalendar-draggable');
            }
            if (isResizable) {
                classes.push('visionCalendar-resizable');
            }
...
// A defined `.eventProps`, even when empty, indicates that an event should be created.
    ...
    var DayGrid = Grid.extend({
        ...
        render: function (isRigid) {
            ...
            this.rowEls = this.el.find('.visionCalendar-row');
            this.dayEls = this.el.find('.visionCalendar-day');
        ...
        dayRowHtml: function (row, isRigid) {
            var view = this.view;
            var classes = ['visionCalendar-row', 'visionCalendar-week', view.widgetContentClass];

            if (isRigid) {
                classes.push('visionCalendar-rigid');
            }

            return '' +
                    '<div class="' + classes.join(' ') + '">' +
                    '<div class="visionCalendar-bg">' +
                    '<table>' +
                    this.rowHtml('day', row) + // leverages RowRenderer. calls dayCellHtml()
                    '</table>' +
                    '</div>' +
                    '<div class="visionCalendar-content-skeleton">' +
                    '<table>' +
                    (this.numbersVisible ?
                            '<thead>' +
        ...
        renderHelper: function (event, sourceSeg) {
            ...
            this.rowEls.each(function (row, rowNode) {
                var rowEl = $(rowNode); // the .visionCalendar-row
                var skeletonEl = $('<div class="visionCalendar-helper-skeleton"><table/></div>'); // will be absolutely positioned
                var skeletonTop;
                ...
                if (sourceSeg && sourceSeg.row === row) {
                    ...
                    skeletonTop = rowEl.find('.visionCalendar-content-skeleton tbody').position().top;
                }
        ...
        renderFillRow: function (type, seg) {
            ...
            skeletonEl = $(
                    '<div class="visionCalendar-' + type.toLowerCase() + '-skeleton">' +
                    '<table><tr/></table>' +
                    '</div>'
                    );
    ...
    DayGrid.mixin({
        ...
        renderFgSegs: function (segs) {
            ...
            this.rowEls.each(function (i, rowNode) {
                $(rowNode).find('.visionCalendar-content-skeleton > table').append(
        ...
        fgSegHtml: function (seg, disableResizing) {
            ...
            classes.unshift('visionCalendar-day-grid-event');

            // Only display a timed events time if it is the starting segment
            if (!event.allDay && seg.isStart) {
                timeHtml = '<span class="visionCalendar-time">' + htmlEscape(this.getEventTimeText(event)) + '</span>';
            }

            titleHtml =
                    '<span class="visionCalendar-title">' +
            ...
            return '<a class="' + classes.join(' ') + '"' +
                    ...
                    '>' +
                    '<div class="visionCalendar-content">' +
                    (this.isRTL ?
                    ...
                    '</div>' +
                    (isResizable ?
                            '<div class="visionCalendar-resizer"/>' :
                            ''
                            ) +
                    '</a>';
        },
            ...
            for (i = 0; i < levelCnt; i++) { // iterate through all levels
                ...
                if (levelSegs) {
                    for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
                        ...
                        td = $('<td class="visionCalendar-event-container"/>').append(seg.el);
...
// NOTE: all the segs being passed around in here are foreground segs
    ...
    DayGrid.mixin({
        ...
        computeRowLevelLimit: function (row) {
            ...
            for (i = 0; i < trEls.length; i++) {
                trEl = trEls.eq(i).removeClass('visionCalendar-limited'); // get and reveal
        ...
        limitRow: function (row, levelLimit) {
            ...
            if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
                ...
                limitedNodes = rowStruct.tbodyEl.children().slice(levelLimit) // get level <tr> elements past the limit
                        .addClass('visionCalendar-limited').get(); // hide elements and get a simple DOM-nodes array
                ...
                for (i = 0; i < levelSegs.length; i++) {
                    ...
                    if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
                        ...
                        for (j = 0; j < colSegsBelow.length; j++) {
                            moreTd = $('<td class="visionCalendar-more-cell"/>').attr('rowspan', rowspan);
                        ...
                        td.addClass('visionCalendar-limited').after($(segMoreNodes)); // hide original <td> and inject replacements
        ...
        unlimitRow: function (row) {
            ...
            if (rowStruct.limitedEls) {
                rowStruct.limitedEls.removeClass('visionCalendar-limited');
        ...
        renderMoreLink: function (cell, hiddenSegs) {
            ...
            return $('<a class="visionCalendar-more"/>')
                    .text(
        ...
        showSegPopover: function (cell, moreLink, segs) {
            ...
            options = {
                className: 'visionCalendar-more-popover',
        ...
        renderSegPopoverContent: function (cell, segs) {
            ...
            var content = $(
                    '<div class="visionCalendar-header ' + view.widgetHeaderClass + '">' +
                    '<span class="visionCalendar-close ' +
                    (isTheme ? 'ui-icon ui-icon-closethick' : 'visionCalendar-icon visionCalendar-icon-x') +
                    '"></span>' +
                    '<span class="visionCalendar-title">' +
                    htmlEscape(title) +
                    '</span>' +
                    '<div class="visionCalendar-clear"/>' +
                    '</div>' +
                    '<div class="visionCalendar-body ' + view.widgetContentClass + '">' +
                    '<div class="visionCalendar-event-container"></div>' +
                    '</div>'
                    );
            var segContainer = content.find('.visionCalendar-event-container');
            var i;
    ...
    var TimeGrid = Grid.extend({
        ...
        render: function () {
            ...
            this.dayEls = this.el.find('.visionCalendar-day');
            this.slatEls = this.el.find('.visionCalendar-slats tr');
        ...
        renderHtml: function () {
            return '' +
                    ...
                    '</table>' +
                    '</div>' +
                    '<div class="visionCalendar-slats">' +
                    '<table>' +
                    this.slatRowHtml() +
                    '</table>' +
                    '</div>';
        ...
        slatRowHtml: function () {
            ...
            while (slotTime < this.maxTime) {
                ...
                axisHtml =
                        '<td class="visionCalendar-axis visionCalendar-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
                ...
                html +=
                        '<tr ' + (!minutes ? '' : 'class="visionCalendar-minor"') + '>' +
                        (!isRTL ? axisHtml : '') +
        ...
        renderHelper: function (event, sourceSeg) {
            ...
            this.helperEl = $('<div class="visionCalendar-helper-skeleton"/>')
                    .append(tableEl)
                    .appendTo(this.el);
        },
        ...
        renderFill: function (type, segs, className) {
            ...
            if (segs.length) {
                ...
                skeletonEl = $(
                        '<div class="visionCalendar-' + className + '-skeleton">' +
                        '<table><tr/></table>' +
                        '</div>'
                        );
                trEl = skeletonEl.find('tr');
                ...
                for (col = 0; col < segCols.length; col++) {
                    ...
                    if (colSegs.length) {
                        containerEl = $('<div class="visionCalendar-' + className + '-container"/>').appendTo(tdEl);
    ...
    TimeGrid.mixin({
        ...
        renderFgSegs: function (segs) {
            ...
            this.el.append(
                    this.eventSkeletonEl = $('<div class="visionCalendar-content-skeleton"/>')
        ...
        renderSegTable: function (segs) {
            ...
            for (col = 0; col < segCols.length; col++) { // iterate each column grouping
                ...
                containerEl = $('<div class="visionCalendar-event-container"/>');
                ...
                for (i = 0; i < colSegs.length; i++) {
                    ...
                    if (seg.bottom - seg.top < 30) {
                        seg.el.addClass('visionCalendar-short');
                    }
        ...
        fgSegHtml: function (seg, disableResizing) {
            ...
            classes.unshift('visionCalendar-time-grid-event');
            ...
            return '<a class="' + classes.join(' ') + '"' +
                    ...
                    (event.title ?
                            '<div class="visionCalendar-title">' +
        ...
        return seg2.forwardPressure - seg1.forwardPressure ||
            ...
            var View = fc.View = Class.extend({
                ...
                initTheming: function () {
                    var tm = this.opt('theme') ? 'ui' : 'visionCalendar';
            ...
            function Calendar(element, instanceOptions) {
                ...
                function initialRender() {
                    tm = options.theme ? 'ui' : 'visionCalendar';
                    element.addClass('visionCalendar');

                    if (options.isRTL) {
                        element.addClass('visionCalendar-rtl');
                    } else {
                        element.addClass('visionCalendar-ltr');
                    }

                    if (options.theme) {
                        element.addClass('ui-widget');
                    } else {
                        element.addClass('visionCalendar-unthemed');
                    }

                    content = $("<div class='visionCalendar-view-container'/>").prependTo(element);
                ...
                function destroy() {
                    ...
                    header.destroy();
                    content.remove();
                    element.removeClass('visionCalendar visionCalendar-ltr visionCalendar-rtl visionCalendar-unthemed ui-widget');
                ...
                function renderView(delta, viewType) {
                    ...
                    if (!currentView && viewType) {
                        ...
                        currentView.el = $("<div class='visionCalendar-view visionCalendar-" + viewType + "-view' />").appendTo(content);
...
// TODO: rename all header-related things to "toolbar"
            ...
            function Header(calendar, options) {
                ...
                function render() {
                    ...
                    if (sections) {
                        el = $("<div class='visionCalendar-toolbar'/>")
                                .append(renderSection('left'))
                                .append(renderSection('right'))
                                .append(renderSection('center'))
                                .append('<div class="visionCalendar-clear"/>');
                ...
                function renderSection(position) {
                    var sectionEl = $('<div class="visionCalendar-' + position + '"/>');
                    ...
                    if (buttonStr) {
                        $.each(buttonStr.split(' '), function (i) {
                                ...
                                if (buttonName == 'title') {
                                    ...
                                    if (buttonClick) {
                                        ...
                                        if (viewText || customText) {
                                            ...
                                            innerHtml = "<span class='visionCalendar-icon visionCalendar-icon-" + normalIcon + "'></span>";
                                        } else {
                                            innerHtml = htmlEscape(defaultText || buttonName);
                                        }

                                        classes = [
                                            'visionCalendar-' + buttonName + '-button',
                                            tm + '-button',
                                            tm + '-state-default'
                                        ];
                            ...
                            if (groupChildren.length > 1) {
                                groupEl = $('<div/>');
                                if (isOnlyButtons) {
                                    groupEl.addClass('visionCalendar-button-group');
                                }
                ...
                function activateButton(buttonName) {
                    el.find('.visionCalendar-' + buttonName + '-button')
            ...
            function EventManager(options) { // assumed to be a calendar
                ...
                function buildEventFromInput(input, source) {
                    ...
                    out._id = input._id || (input.id === undefined ? '_visionCalendar' + eventGUID++ : input.id + '');
                ...
                function getBusinessHoursEvents() {
                    ...
                    var defaultVal = {
                        className: 'visionCalendar-nonbusiness',
                        start: '09:00',
                        end: '17:00',
...
// It is responsible for managing width/height.
            ...
            var BasicView = fcViews.basic = View.extend({
                ...
                render: function () {
                    ...
                    this.el.addClass('visionCalendar-basic-view').html(this.renderHtml());

                    this.headRowEl = this.el.find('thead .visionCalendar-row');

                    this.scrollerEl = this.el.find('.visionCalendar-day-grid-container');
                    this.dayGrid.coordMap.containerEl = this.scrollerEl; // constrain clicks/etc to the dimensions of the scroller

                    this.dayGrid.el = this.el.find('.visionCalendar-day-grid');
                ...
                renderHtml: function () {
                    return '' +
                            ...
                            '<div class="visionCalendar-day-grid-container">' +
                            '<div class="visionCalendar-day-grid"/>' +
                            '</div>' +
                            '</td>' +
                            '</tr>' +
                            '</tbody>' +
                ...
                headIntroHtml: function () {
                    if (this.weekNumbersVisible) {
                        return '' +
                                '<th class="visionCalendar-week-number ' + this.widgetHeaderClass + '" ' + this.weekNumberStyleAttr() + '>' +
                ...
                numberIntroHtml: function (row) {
                    if (this.weekNumbersVisible) {
                        return '' +
                                '<td class="visionCalendar-week-number" ' + this.weekNumberStyleAttr() + '>' +
                                '<span>' + // needed for matchCellWidths
                ...
                dayIntroHtml: function () {
                    if (this.weekNumbersVisible) {
                        return '<td class="visionCalendar-week-number ' + this.widgetContentClass + '" ' +
                ...
                introHtml: function () {
                    if (this.weekNumbersVisible) {
                        return '<td class="visionCalendar-week-number" ' + this.weekNumberStyleAttr() + '></td>';
                    }
                },
                ...
                numberCellHtml: function (cell) {
                    ...
                    classes.unshift('visionCalendar-day-number');
                ...
                updateWidth: function () {
                    if (this.weekNumbersVisible) {
                        ...
                        this.weekNumberWidth = matchCellWidths(
                                this.el.find('.visionCalendar-week-number')
                                );
                    }
                },
            ...
            ;
            ;
            /* A Month view with simple day cells running horizontally
             ----------------------------------------------------------------------------------------------------------------------*/
...
// Responsible for managing width/height.
            ...
            fcViews.agenda = View.extend({// AgendaView
                ...
                render: function () {
                    ...
                    this.el.addClass('visionCalendar-agenda-view').html(this.renderHtml());

                    // the element that wraps the time-grid that will probably scroll
                    this.scrollerEl = this.el.find('.visionCalendar-time-grid-container');
                    this.timeGrid.coordMap.containerEl = this.scrollerEl; // don't accept clicks/etc outside of this

                    this.timeGrid.el = this.el.find('.visionCalendar-time-grid');
                    ...
                    this.noScrollRowEls = this.el.find('.visionCalendar-row:not(.visionCalendar-scroller *)'); // fake rows not within the scroller
                },
                ...
                renderHtml: function () {
                    return '' +
                            ...
                            '<div class="visionCalendar-time-grid-container">' +
                            '<div class="visionCalendar-time-grid"/>' +
                            '</div>' +
                            '</td>' +
                            '</tr>' +
                            '</tbody>' +
                ...
                headIntroHtml: function () {
                    ...
                    if (this.opt('weekNumbers')) {
                        ...
                        return '' +
                                '<th class="visionCalendar-axis visionCalendar-week-number ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '>' +
                                '<span>' + // needed for matchCellWidths
                                htmlEscape(weekText) +
                                '</span>' +
                                '</th>';
                    } else {
                        return '<th class="visionCalendar-axis ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '></th>';
                    }
                },
                ...
                dayIntroHtml: function () {
                    return '' +
                            '<td class="visionCalendar-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + '>' +
                            '<span>' + // needed for matchCellWidths
                ...
                slotBgIntroHtml: function () {
                    return '<td class="visionCalendar-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + '></td>';
                },
                ...
                introHtml: function () {
                    return '<td class="visionCalendar-axis" ' + this.axisStyleAttr() + '></td>';
                },
                ...
                updateWidth: function () {
                    ...
                    this.axisWidth = matchCellWidths(this.el.find('.visionCalendar-axis'));
                },
