...
;(function($) {
    "use strict";
    var cronInputs={
        period: '<div class="cron-select-period"><label></label><select style="display:none" type="hidden" class="cron-period-select"></select></div>',
...
//                '<p><input type="radio" name="hourlyType" value="every"> Every <select class="cron-hourly-select"></select> hour(s)</p>',
                '<p><input type="radio" name="hourlyType" value="clock">Select Time to Unlock <select class="cron-hourly-hour"></select>:<select class="cron-hourly-minute"></select></p>']
        },
        daily: {
            tag: 'cron-daily',
    ...
    var periodOpts=arrayToOptions([ "Hourly"]);
    ...
    $.LOCK = function(el, options) {
        ...
        base.$el.data('LOCK', base);
        
        // Initialization
        base.init = function() {
            base.options = $.extend({},$.LOCK.defaultOptions, options);
