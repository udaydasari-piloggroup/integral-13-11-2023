...
if (isSafari && maxVolume==-100){
    if (fftBinsErrorFlag==false){
        fftBinsErrorFlag=true;
    } else{
  harkErrorFlag= true;
throw new Error('HarkError');
    }
}
  return maxVolume;
}
...
module.exports = function(actualStream, options) {
    stream = actualStream.clone();
    //actualStream.getAudioTracks()[0].stop();
